...
export function PoliedroPrismaTriangularIsoscelesForm({ setValue }: FormProps) {
      ...
      <form
        onSubmit={form.handleSubmit((v: FormSchema) => {
          ...
          setValue({
            ...
            area:
              2 * baseArea +
              (v.arestaBaseBase + 2 * Math.sqrt(Math.pow(v.arestaBaseAltura, 2) + Math.pow(v.arestaBaseBase / 2, 2))) *
                v.altura,
          });
        })}
