...
export function PoliedroPiramideTriangularEscalenoForm({ setValue }: FormProps) {
  ...
  return (
    <Form {...form}>
      ...
      <p>
        A = A<sub>b</sub> + ((b + √(c<sup>2</sup> + (b / 2)<sup>2</sup>) + √(c<sup>2</sup> + (b / 2)<sup>2</sup>)) × h)
        / 2
      </p>
      <hr />
      <form
        onSubmit={form.handleSubmit((v: FormSchema) => {
          ...
          setValue({
            ...
            area:
              baseArea +
              ((v.arestaBaseBase +
                Math.sqrt(Math.pow(v.arestaBaseAltura, 2) + Math.pow(v.arestaBaseBase / 2, 2)) +
                Math.sqrt(Math.pow(v.arestaBaseAltura, 2) + Math.pow(v.arestaBaseBase / 2, 2))) *
                v.altura) /
                2,
          });
