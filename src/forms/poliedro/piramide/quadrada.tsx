...
export function PoliedroPiramideQuadradaForm({ setValue }: FormProps) {
  ...
  return (
    <Form {...form}>
      <p>
        A<sub>b</sub> = a<sup>2</sup>
      </p>
      <p>
        V = (A<sub>b</sub> × h) / 3
      </p>
      <p>
        A = A<sub>b</sub> + 2 × a × √(h<sup>2</sup> + (a / 2)<sup>2</sup>)
      </p>
      <hr />
      <form
        onSubmit={form.handleSubmit((v: FormSchema) => {
          const baseArea = Math.pow(v.arestaBase, 2);
          setValue({
            volume: (baseArea * v.altura) / 3,
            area: baseArea + 2 * v.arestaBase * Math.sqrt(Math.pow(v.altura, 2) + Math.pow(v.arestaBase / 2, 2)),
          });
        })}
