...
export function CorpoRedondoConeForm({ setValue }: FormProps) {
  ...
  return (
    <Form {...form}>
      ...
      <p>
        V = (A<sub>b</sub> × h) / 3 = (π × r<sup>2</sup> × h) / 3
      </p>
      <p>
        A = π × r × (r + √(r<sup>2</sup> + h<sup>2</sup>))
      </p>
      <hr />
      <form
        onSubmit={form.handleSubmit((v: FormSchema) => {
          setValue({
            volume: (Math.PI * Math.pow(v.raio, 2) * v.altura) / 3,
            area: Math.PI * v.raio * (v.raio + Math.sqrt(Math.pow(v.raio, 2) + Math.pow(v.altura, 2))),
          });
        })}
